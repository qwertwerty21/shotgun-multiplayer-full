...
class ResultScreen extends Component {
    ...
    componentWillReceiveProps(nextProps){
		console.log('COMPONENT RESULTSCREEN WILL REVCEIVE PROPS CHECK ME', nextProps)
    ...
    renderNextRoundButton() {
        ...
        if (playerIsDead && this.props.game.room.playing){
        ...
        else if (!playerIsDead && this.props.game.room.playing && !this.state.nextRoundBtnClicked) {
            return ( 
            ...
            return(
                <div className="nextround__waitingnotice">Waiting for Other Players to Begin Next Round...</div>
            )
        }
        //if playing set to false add a button which redirects to homescreen
        else {
            return ( 
                <RaisedButton label = 'Return to Homescreen'
                primary = { true }
                fullWidth = { true }
