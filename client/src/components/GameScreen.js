...
class GameScreen extends Component{
	...
	componentWillReceiveProps(nextProps){
		...
		else if(nextProps.currentPlayer && nextProps.currentPlayer.currentMove && !nextProps.currentPlayer.currentMoveReceived && nextProps.game.room.results.length < 1){
	...
	componentDidMount() {
		...
		document.getElementById('gamescreenVideo').addEventListener('loadedmetadata', function() {
	...
	renderPlayersAlive(){
		...
		if(this.props.game.room.playersAlive && this.props.game.room.results.length < 1){
    ...
    renderShootMenuItems(){
		if(this.props.game.room.playersAlive && this.props.game.room.results.length < 1){
			let allPlayersAliveExceptCurrent = this.props.game.room.playersAlive.filter((player, index)=>{
			...
			return allPlayersAliveExceptCurrent.map((player, index)=>{
				return(
					<MenuItem
						...
						value={player.id}
						key={index}
						onTouchTap={this.handleCurrentMove.bind(this, {move: 'shoot', target: player.id })}
					>
	...
	renderPlayerMoveBtns(){
		...
		console.log('HI THIS THIS THE CURRENT PLAYER WHEN RENDERING PLAYER MOVE BTNS', this.props.currentPlayer)
		if(playerIsDead){
			return(
				<h3 className="playermove__deadnotice">You're Dead, buddy...</h3>
			)
		}
		else if(!playerIsDead && this.props.currentPlayer.currentMoveReceived || this.state.moveBtnClicked ){
			return(
				...
				<List>
					...
					<ListItem>
						<RaisedButton		
							...
							primary={true}
							fullWidth={true}
							onTouchTap={this.handleCurrentMove.bind(this, {move: 'reload', target: null})} //dispatch action init game to server
						>		
						</RaisedButton>
					</ListItem>
					<ListItem>
						<RaisedButton		
							...
							primary={true}
							fullWidth={true}
							onTouchTap={this.handleCurrentMove.bind(this, {move: 'block', target: null})} //dispatch action init game to server
						>		
						</RaisedButton>
					</ListItem>
				...
				<div className="hud__container" >
					...
					<div>
						...
						<span>
							...
							{this.props.currentPlayer.bullets}
						</span>
					</div>
					<div>
						<span className="hud__label">
	...
	render(){
		...
		return(
			...
			<div className="gamescreen__container">
				...
				<video playsinline autoplay muted loop className="gamescreen__video" id="gamescreenVideo">
				...
				<div className='row'>
					<div className="col-md-6 col-md-offset-3 text-center">
						...
						<h3 className="playersremain__header">
							Players Remaining:	
						</h3>
